...
@endphp
<li class="mb-2 ms-5">
    <span class="absolute -start-[7px] mt-2 h-3 w-3 rounded-full ring-4 ring-background {{ $dot }}"></span>
    <div @if ($isFocus) id="al-focus-entry" @endif
         class="rounded-xl border shadow-xs transition-colors {{ $isFocus ? 'border-brand ring-2 ring-brand/30 bg-brand/5' : ($entry->isNoise() ? 'border-warning/40 bg-warning/5' : 'border-border bg-card') }}">
        ...
        @if ($entry->changeCount() > 0)
            <div id="{{ $diffId }}" class="{{ $isFocus ? '' : 'hidden' }} border-t border-border px-4 py-3 animate-slide-down">
